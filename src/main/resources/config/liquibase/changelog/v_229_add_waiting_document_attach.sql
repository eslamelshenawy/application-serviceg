...
VALUES ((select max(id) + 1 from application.lk_application_status),'1',0,'بانتظار ارفاق الوثائق','Waiting Documents Attachment','WAITING_DOCUMENTS_ATTACHMENT','بانتظار ارفاق الوثائق','Waiting Documents Attachment',
        (select id FROM application.lk_application_category x
         WHERE x.saip_code = 'INDUSTRIAL_DESIGN'));
