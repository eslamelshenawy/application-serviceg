...
select
	...
	where
		lss.code = 'EVICTION'
		and lac.saip_code = 'INDUSTRIAL_DESIGN') lk_support_service_type_id,
...
from
	...
	from
		application.lk_support_service_request_status lssrs
	where
		lssrs.code in ('DRAFT', 'UNDER_PROCEDURE', 'REJECTED', 'COMPLETED', 'WAIVED'))) lk_support_service_status_id;
--------------------------------------------------------------------------------------------------------------------------
insert
	into
	application.support_service_type_status
select
	...
	join application.lk_application_category lac on
		ssac.category_id = lac.id
	where
		lss.code = 'REVOKE_BY_COURT_ORDER'
		and lac.saip_code = 'INDUSTRIAL_DESIGN') lk_support_service_type_id,
	lk_support_service_status_id
from
	unnest(array(
